...
def load_map(mapFilename):
    ...
    dir_graph = WeightedDigraph()

    with open(mapFilename) as input_file:
        for line in input_file.readlines():
            src, dest, total, outdoors = line.split()
            srcNode = Node(src)
            destNode = Node(dest)
            if not dir_graph.hasNode(srcNode):
                dir_graph.addNode(srcNode)
            if not dir_graph.hasNode(destNode):
                dir_graph.addNode(destNode)
            edge = WeightedEdge(srcNode, destNode, total, outdoors)
            dir_graph.addEdge(edge)
...
#print "Nodes printed:" + str(mitMap.nodes)
#print "Edges printed:" + str(mitMap.edges)
