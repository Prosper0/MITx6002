...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for me_point in self.members():
            for other_point in other.members():
                curr_dist = me_point.distance(other_point)
    ...
    def averageLinkageDist(self, other):
        ...
        tot_num_points = len(self.points) * len(other.points)
        
        for me_point in self.members():
            for other_point in other.members():
                avg_dist += me_point.distance(other_point)
                
        return avg_dist / float(tot_num_points)
    def members(self):
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        for curr_point1 in c1.members():
            total_points.append(curr_point1)
            
        for curr_point2 in c2.members():
            total_points.append(curr_point2)
            
        union_cluster = Cluster(total_points, c1.pointType)
    ...
    def findClosest(self, linkage):
        ...
        for cluster_1 in self.members:
            for cluster_2 in self.members:
                if cluster_1 != cluster_2:
                    curr_dist = linkage(cluster_1, cluster_2)
                    if curr_dist < closest_dist:
                        closest_dist = curr_dist
                        closest_tuple = (cluster_1, cluster_2)
    ...
    def mergeOne(self, linkage):
        ...
        cluster_1, cluster_2 = self.findClosest(linkage)
        self.mergeClusters(cluster_1, cluster_2)
...
def test():
    points = buildCityPoints(r'F:\Dev\Python\MITx6002\W06\ProblemSet6\cityTemps.txt', True)
...
def simple_test():
    points = buildCityPoints(r'F:\Dev\Python\MITx6002\W06\ProblemSet6\test.txt', False)
